/*
 * Gap Analysis Page — PEI Dashboard
...
export default function GapAnalysisPage() {
  ...
  const gapSummary = useMemo(() => {
    const categories = [
      { key: "salesPro", label: "Sales Pro", color: GAP_COLORS.salesPro },
      { key: "techPro", label: "Tech Pro", color: GAP_COLORS.techPro },
      { key: "bootcamp", label: "Bootcamp", color: GAP_COLORS.bootcamp },
      { key: "implSpec", label: "Impl Specialist", color: GAP_COLORS.implSpec },
    ];
    ...
    return categories.map((cat) => {
      ...
      const partnersWithGap = modifiedPartners.filter((p) => {
        const req = p.requirements[cat.key as keyof typeof p.requirements];
        return req.required > 0 && req.obtained < req.required;
      });
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-[13px] text-muted-foreground mt-1">
          {totalGaps} total enablement gaps across {modifiedPartners.length} partners — detailed breakdown by category
        </p>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-[13px] font-bold" style={{ color: cat.color }}>{cat.label}</h4>
              ...
              <AnimatePresence>
                {isExpanded && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="mt-3 pt-3 border-t border-border/40 space-y-1">
                    <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold mb-1">Partners with gaps</p>
                    {cat.partners.map((p) => {
                      const req = p.requirements[cat.key as keyof typeof p.requirements];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        {/* Stacked Bar */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="terrain-card p-5 lg:col-span-3">
          <h3 className="text-[14px] font-bold text-foreground mb-1">Gap Distribution by Partner</h3>
          <p className="text-[11px] text-muted-foreground mb-4">Stacked gaps across all four requirement categories</p>
        ...
        {/* Gap Proportion Pie */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="terrain-card p-5 lg:col-span-2">
          <h3 className="text-[14px] font-bold text-foreground mb-1">Gap Proportion</h3>
          <p className="text-[11px] text-muted-foreground mb-4">Share of total gaps by category</p>
          <div className="h-[220px]">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={pieData.filter((d) => d.value > 0)} cx="50%" cy="50%" innerRadius={55} outerRadius={85} paddingAngle={3} dataKey="value">
                  ...
                  content={({ payload }) => {
                    ...
                    return (
                      <div className="terrain-card px-3 py-2 shadow-lg">
                        ...
                        <p className="text-[10px] text-muted-foreground">{d.value} gaps ({totalGaps > 0 ? Math.round((d.value / totalGaps) * 100) : 0}%)</p>
                      </div>
                    );
                  }}
                />
          ...
          <div className="space-y-2 mt-2">
            {pieData.map((d) => (
              <div key={d.name} className="flex items-center justify-between text-[11px]">
                ...
                <span className="font-bold text-muted-foreground">{d.value} ({totalGaps > 0 ? Math.round((d.value / totalGaps) * 100) : 0}%)</span>
              </div>
            ))}
          </div>
        </motion.div>
          ...
          <table className="w-full text-[11px]">
            ...
            <tbody>
              {heatmapData.map((row) => {
                ...
                const def = TIER_DEFINITIONS[row.programTier];
                return (
                  <tr key={row.id} className="border-b border-border/20 hover:bg-black/[0.02] transition-colors">
                    ...
                    <td className="py-2 px-3 text-center">
                      <span className="text-[9px] font-bold px-1.5 py-0.5 rounded-full" style={{ background: def.bg, color: def.color }}>
                        {def.label}
                      </span>
                    </td>
