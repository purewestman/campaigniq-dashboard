          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b" style={{ borderColor: 'oklch(0.58 0.16 290 / 0.12)' }}>
                ...
                <th className="py-3 px-4 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider text-center">Completed</th>
            ...
            <tbody className="divide-y divide-black/5">
              {trainingData.map((row) => (
                <tr key={row.id} className="hover:bg-black/[0.02] transition-colors">
                  ...
                  <td className="py-4 px-4 text-center">
                    <div className="flex items-center justify-center gap-1.5 font-semibold text-[13px]">
                      {row.count}
                  ...
                  <td className="py-4 px-4 text-center">
                    {row.rating === "Expert" && (
                      <Badge className="bg-purple-100 text-purple-700 hover:bg-purple-200">Expert</Badge>
                    )}
                    {row.rating === "Proficient" && (
                      <Badge className="bg-teal-100 text-teal-700 hover:bg-teal-200">Proficient</Badge>
                    )}
                    {row.rating === "Novice" && (
                      <Badge className="bg-blue-100 text-blue-700 hover:bg-blue-200">Novice</Badge>
                    )}
                    {row.rating === "None" && (
                      <Badge variant="outline" className="text-muted-foreground border-dashed">No Training</Badge>
                    )}
                  </td>
                  <td className="py-4 px-4">
                    {row.isNextToSpeakTo ? (
                      <div className="flex items-center gap-1.5 text-[12px] font-semibold text-rose-600 bg-rose-50 w-max px-2.5 py-1 rounded-full border border-rose-100">
              ...
              {trainingData.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-8 text-center text-[13px] text-muted-foreground">
